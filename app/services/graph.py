...
def _judge_prompt(mode: str) -> str:
    ...
    return (
        ...
        f"{mode_guide}\n\n"
        "【조항 인용 원칙 — 반드시 준수】\n"
        "- 근거는 법령 조항(법률·시행령·감독규정·지정고시)을 우선 인용하고, 제공된 조항의 article 표기 그대로 정확한 조·항·호까지 명시할 것\n"
        "- 금감원 가이드라인·협회 준칙 등 비법령 자료(id 가 GUIDE-·FAG-·ASSOC-·AUX- 로 시작)는 해당 위반을 직접 규율하는 법령 조항이 없을 때만 보조로 인용하고, 법령 조항이 있으면 그것을 우선할 것\n"
        "- 가장 직접적이고 구체적인 하위규정을 1순위로 인용: 감독규정 > 시행령 > 지정고시·가이드라인 > 상위법\n"
        "- '누구나', '누구든지', '묻지마' 등 보편적용 오인 표현의 직접 근거는 반드시 감독규정 제19조(FCPA-감독규정19). "
        "FSCPA-21-3·21-4는 부당비교(금소법 제21조 제4호) 조항이므로 보편적용 오인에는 인용하지 말 것\n"
        "- '업계 1위', '최고', '유일' 등 최상급의 직접 근거는 공정위 지정고시(FTC-지정고시)\n"
        "- '무조건', '반드시', '확실히' 등 단정적 표현의 직접 근거는 금소법 제21조 제1호(FCPA-21-단정)\n"
        "- '선착순', '한정', '마감 임박' 등 희소성·긴박성 표현의 직접 근거는 공정위 지정고시(FTC-지정고시). "
        "다크패턴 청크(DARK-15 순차공개 가격책정 등)는 희소성 표현의 근거로 인용하지 말 것\n"
