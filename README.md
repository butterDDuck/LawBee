...
- 새로 검수하기로 콘텐츠를 제출하면 AI 1차 심의 실행 (텍스트·이미지·영상)
- 검수 상세에서 심의 결과를 검토하고 승인·조건부승인·반려를 기록

### 멀티모달 심의

- **이미지** — OpenAI Vision 으로 문구·시각 구성을 추출하여 심의 (`POST /reviews/image`)
- **영상** — Whisper 로 자막을 추출하여 구간별 심의 후 위반 문구를 타임라인에 매핑 (`POST /reviews/video`)
